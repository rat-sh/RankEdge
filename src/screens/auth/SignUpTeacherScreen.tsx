...
const SignUpTeacherScreen = () => {
  ...
  const [form, setForm] = useState({ name: '', email: '', phone: '', institution: '', city: '', pin: '', confirmPin: '' });
  ...
  const handleSignUp = async () => {
    ...
    if (selectedSubjects.length === 0) { Alert.alert('Error', 'Select at least one subject'); return; }
  ...
  return (
    <ScrollView style={s.root} contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
      ...
      <Text style={s.title}>Create Teacher Account</Text>
      <Text style={s.subtitle}>Set up your RankEdge profile</Text>
      ...
      {[
        ...
        { key: 'institution', placeholder: 'Institution / School (optional)', keyboard: 'default' },
      ...
      <View style={s.chips}>
        {SUBJECTS.map(sub => (
          <TouchableOpacity key={sub} style={[s.chip, selectedSubjects.includes(sub) && s.chipSelected]} onPress={() => toggleSubject(sub)}>
            <Text style={[s.chipText, selectedSubjects.includes(sub) && s.chipTextSelected]}>{sub}</Text>
          </TouchableOpacity>
        ))}
      </View>
...
const s = StyleSheet.create({
  ...
  input: { backgroundColor: '#22304A', borderRadius: 12, padding: 14, color: '#fff', fontSize: 15, marginBottom: 14 },
  sectionLabel: { color: '#8899BB', fontSize: 13, fontWeight: '600', marginBottom: 10, marginTop: 6 },
  chips: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 20 },
  chip: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, borderWidth: 1.5, borderColor: '#334466' },
  chipSelected: { backgroundColor: '#4F81BD', borderColor: '#4F81BD' },
  chipText: { color: '#8899BB', fontSize: 13, fontWeight: '600' },
  chipTextSelected: { color: '#fff' },
  btn: { backgroundColor: '#4F81BD', borderRadius: 14, paddingVertical: 16, alignItems: 'center', marginTop: 8, marginBottom: 16 },
